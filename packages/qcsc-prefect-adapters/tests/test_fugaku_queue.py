...
def _pjstat_limit_output(*rows: str, group: str = "ra010014") -> str:
    return (
        "\n".join(
            [
                f" GROUP: {group}",
...
def test_estimate_native_bulk_capacity_prefers_bulk_subjob_metric():
    capacity = queue_mod.estimate_capacity_from_pjstat_limit(
        _PJSTAT_LIMIT_SAMPLE,
        max_active_jobs=1000,
        group="ra010014",
    ...
    assert capacity.max_active_jobs == 2000
    assert capacity.current_active_jobs == 0
    assert capacity.available_slots == 2000
    assert capacity.raw_output == _PJSTAT_LIMIT_SAMPLE


def test_estimate_native_bulk_capacity_falls_back_to_allsubjob_metric():
    output = _pjstat_limit_output(
        _limit_row("ru-accept", "1000", 7),
        _limit_row("ru-accept-allsubjob", "1200", 9),
    )
    ...
    assert capacity.max_active_jobs == 1200
    assert capacity.current_active_jobs == 9
    assert capacity.available_slots == 1191
    assert capacity.raw_output == output


def test_estimate_native_bulk_capacity_falls_back_to_ru_accept_metric():
    output = _pjstat_limit_output(_limit_row("ru-accept", "1000", 7))
...
def test_estimate_capacity_uses_explicit_limit_name_without_fallback():
    output = _pjstat_limit_output(
        _limit_row("ru-accept", "1000", 7),
        _limit_row("ru-accept-allsubjob", "1200", 9),
    )
    ...
    capacity = queue_mod.estimate_capacity_from_pjstat_limit(
        ...
        max_active_jobs=1000,
        group="ra010014",
        limit_name="ru-accept-allsubjob",
    )
...
def test_fugaku_queue_probe_native_bulk_reads_bulk_subjob_capacity(monkeypatch):
    calls: list[str | None] = []
...
def test_fugaku_queue_probe_native_bulk_reads_allsubjob_fallback(monkeypatch):
    output = _pjstat_limit_output(
        _limit_row("ru-accept", "1000", 7),
        _limit_row("ru-accept-allsubjob", "1200", 9),
    )
...
def test_fugaku_queue_probe_native_bulk_reads_ru_accept_fallback(monkeypatch):
    output = _pjstat_limit_output(_limit_row("ru-accept", "1000", 7))
...
def test_fugaku_queue_probe_missing_explicit_metric_returns_zero_capacity(monkeypatch):
    output = _pjstat_limit_output(_limit_row("ru-accept", "1000", 7))
    ...
    probe = queue_mod.FugakuQueueProbe(
        max_active_jobs=1000,
        project="ra010014",
        limit_name="ru-accept-bulksubjob",
    )

    capacity = probe.get_capacity()

    assert capacity.max_active_jobs == 1000
    assert capacity.current_active_jobs == 1000
    assert capacity.available_slots == 0
    assert capacity.raw_output == output


def test_fugaku_queue_probe_missing_native_bulk_metrics_returns_zero_capacity(monkeypatch):
    output = _pjstat_limit_output(_limit_row("ru-run-job", "1000", 7))
...
def test_fugaku_queue_probe_failure_path_returns_zero_capacity(monkeypatch):
    ...
    assert capacity.available_slots == 0
    assert "partial stdout" in str(capacity.raw_output)
